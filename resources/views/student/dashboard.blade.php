                ...
                <div class="mt-4 space-y-3">
                    @foreach($enrollments as $course)
                        <div class="flex items-center justify-between rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-800">
                            <div>
                                <p class="font-medium text-zinc-900 dark:text-zinc-100">{{ $course->title }}</p>
                                <p class="text-sm text-zinc-500 dark:text-zinc-400">
                                    {{ __('Enrolled') }}: {{ \Carbon\Carbon::parse($course->pivot->enrolled_at)->format('M d, Y') }}
                                </p>
                            </div>
                            <span @class([
                                'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium',
                                'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' => $course->pivot->status === 'paid',
                                'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' => $course->pivot->status === 'pending',
                                'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' => $course->pivot->status === 'cancelled',
                                'bg-zinc-100 text-zinc-800 dark:bg-zinc-700 dark:text-zinc-300' => !in_array($course->pivot->status, ['paid', 'pending', 'cancelled']),
                            ])>
